<?php
    ...
    if ($_SERVER['REQUEST_METHOD']=='POST') {
        ...
        if ($result_qtemp->num_rows > 0) {
            $query_res = "INSERT INTO `reservation`(`id_reservation`, 
            detail_reservation, grandtotal_reservation,
            `date_reservation`, `time_reservation`, `timecollect_reservation`, `id_timelinereserv`, 
            `state_reservation`, `id_customer`, `id_typepay`, `id_dealer`, `id_direccust`) 
        VALUES ('$id_res','$detail_res', '$gtotal','$date','$time', '$time_res', 1, 1, '$id_cus',
            ...
            if (!$result_qres) {
                ...
                $query_idnot = "INSERT INTO `detail_notifications`(`id_detnotifications`, destination_detnotif,
                    `detail_detnotif`, `date_detnotif`, `time_detnotif` ) 
                    VALUES ('$ai','$destination','$msg','$date','$time')";
                ...
                while ($row_qtemp=$result_qtemp->fetch_array()) {
                    ...
                    VALUES ('','$id_res','$qty','$total','$price', '$target', '$id_item', NULL)";
                        ...
                        `stock_detprod`= stock_detprod -'$qty' WHERE id_product = '$id_item'";
